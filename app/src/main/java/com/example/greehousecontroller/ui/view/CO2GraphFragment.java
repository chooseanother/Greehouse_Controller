        ...
        co2GraphViewModel.getCo2HistoryData().observe(getViewLifecycleOwner(), new Observer<List<CO2>>() {
            ...
            public void onChanged(@Nullable List<CO2> co2s) {
                if (co2s != null) {
                    if (co2s.size() > 0) {
                        for (int i = 0; i < 1; i++) {
                            long localTime = (long) Objects.requireNonNull(co2GraphViewModel.getLatestCO2().getValue()).getLocalTime();
                            double measurement = co2GraphViewModel.getLatestCO2().getValue().getCo2Measurement();
                            data.add(new GraphsFragment.OHCLDataEntry(localTime, 0.1, 0.1, 0.1, measurement));
                        }
                        for (CO2 co2 : co2s) {
                            data.add(new GraphsFragment.OHCLDataEntry(co2.getLocalTime(), co2.getCo2Measurement(), co2.getCo2Measurement(), co2.getCo2Measurement(), co2.getCo2Measurement()));
                        }
